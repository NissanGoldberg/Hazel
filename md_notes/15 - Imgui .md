...
* `ImGuiLayer.h` and `ImGuiLayer.h`

New Directories

* `Platform/OpenGL`
  * copy here `imgui_impl_opengl3.h` and `.cpp` 
  * copy here `imgui_impl_glfw.h`  and `.cpp` 
* `src/Hazel/ImGui`

Files Changes:

* `Application.h` and `.cpp`
...
Add to `Hazel.h` : `#include "Hazel/ImGui/ImGuiLayer.h"`
...
public:
	Sandbox() {
		...
		PushOverlay(new Hazel::ImGuiLayer()); //<-- Added this vid 15
	}
